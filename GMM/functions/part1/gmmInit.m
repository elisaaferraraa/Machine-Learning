...
%Priors
K=params.k;
Priors0 = ones(1,K)./K; % k uniformly random numbers
assert(sum(Priors0)-1<1e-5);

%Mu
[labels0, Mu0, Mu_init, iter] =  kmeans(X,params.k,params.init,params.d_type,params.max_iter_init,0);

%Sigma
[N,K]= size(Mu0);
for k = 1 : K
    X_k_labelled = X(:,labels0==k);
    Sigma0(:,:,k)=compute_covariance( X_k_labelled , Mu0(:,k), params.cov_type );
